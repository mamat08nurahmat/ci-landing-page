...
function pipeline_coach_kb()
	...
	// create report in excel
	function xlsPipeline($id = 0, $staging=0,  $start = NOW, $end = NOW)
	{
		$lvl = ($_SESSION['USER_LEVEL'] != '')?$_SESSION['USER_LEVEL']:'';
		$arrlvl = array('SALES');
		#if( ! in_array($lvl,$arrlvl)) {redirect('forbiden/index/1');}
		...
		if($data)
		{
			...
			foreach($data as $row)
			{
				...
				if($product==5)
				{
					...
					$html .= "<td width='' align='right' class='kecil'>".number_format($row->DELTA_BASELINE,2,'.',',')."</td> \n";
				}
				if($product==6)
				{
					$html .= "<td width='' align='right' class='kecil'>".number_format($row->DPK,2,'.',',')."</td> \n";
	...
	function get_performance_new($id =0, $m=0, $y = 0)
	{
		...
		#$this->_report->update_performance_sales($id);
		#$data 	= $this->_report->get_performance($id, $m, $y);
		$data 	= $this->_report->get_performa_sales_new($id, $m, $y);
		$usergrade	= $this->_report->get_user_grade($id,$m,$y);
		//$status = $this->_status->get_status_cek($id);
		#print_r($data); die();
		$salestypenew=$usergrade[0]->SALES_TYPE;
		...
		if($data)
		{
			...
			{
			$fin =0; $fininsentif = 0; $bfin=0; $rfin=0;
			$kik=0; $bkik=0; $rkik=0;
			$x=1;
			$y=0;
			foreach($data as $row)
			{
				...
				{ 
					$fin += $row->REALISASI_TERBOBOT;
					$fininsentif += $row->REALISASI_TERBOBOT_INSENTIF;
					$rfin += $row->BOBOT_CAP;
					$bfin += $row->BOBOT;
					$xfin += $row->REALISASI;
				}
				...
				if($row->BOBOT_CAT != 1 && $x == 1){				
					...
					$html .= "<td width='' align='center'>".$row->BOBOT."%</td> \n";
					$html .= "<td width='' align='center'>".$row->REALISASI."%</td> \n";
					$html .= "<td width='' align='center'>".$row->REALISASI_TH."%</td> \n";
					#$html .= "<td width='' align='center'>".$row->BOBOT_CAP."%</td> \n";
					$html .= "<td width='' align='center'>".$row->REALISASI_TERBOBOT."%</td> \n";
					$html .= "<td width='' align='center'>".$row->REALISASI_TERBOBOT_INSENTIF."%</td> \n";
					#$html .= "<td width='' align='left'  >".$cat."</td> \n";
					$html .= "</tr> \n";
				} else {				
					$html .= "<tr bgcolor='$color'>\n";
				...
				$html .= "<td width='' align='center'>&nbsp;</b></td> \n";
				#$html .= "<td width='' align='center'><b>".number_format(($xfin),1,'.',',')."%</b></td> \n";
				$html .= "<td width='' align='center' ><b>".number_format(($fin),1,'.',',')."%</b></td> \n";
				if($fininsentif>=75)
				{	
					$html .= "<td width='' align='center' bgcolor='#ADFF2F'><b>".number_format(($fininsentif),1,'.',',')."%</b></b></td> \n";
				}
				elseif($fininsentif>=0&&$fininsentif<75)
				{
					$html .= "<td width='' align='center' bgcolor='#FAFAD2'><b>".number_format(($fininsentif),1,'.',',')."%</b></b></td> \n";
				}
				else
				{
					$html .= "<td width='' align='center' bgcolor='#FF6347'><b>".number_format(($fininsentif),1,'.',',')."%</b></b></td> \n";
				}
				#$html .= "<td width='' align='left'  ><b>$cat</b></td> \n";
				$html .= "</tr> \n";
	...
	function get_tunjangan_performance($id =0, $m=0, $y = 0,$salestipe = 0)
	{
		$bulan = array(	'1'=>'Januari', '2'=>'Februari', '3'=>'Maret', '4'=>'April',
                                        '5'=>'Mei', '6'=>'Juni', '7'=>'Juli', '8'=>'Agustus',
                                        '9'=>'September', '10'=>'Oktober', '11'=>'November', '12'=>'Desember');
