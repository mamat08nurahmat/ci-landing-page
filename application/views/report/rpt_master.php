...
<script type="text/javascript">
	$(function() {
		...
		function getReport(ex){
			...
			var wil = $('#WILAYAH').val();
			var msg = '';
			if(id == '') {alert('NPP tidak boleh kosong !'); return false;}
			else {
				if(ex == 0){
					<?php if(isset($js) && $js==0) { ?>
					var urls = '<?php echo $url_data ?>/0/'+ month +'/'+year;
					<?php } else { ?>
					var urls = '<?php echo $url_data ?>/0/'+ id +'/'+year;
					<?php } ?>
					$("#report").html("<img src='<?php echo LOAD ?>'> <span style='color:#080'>loading</span>");
					$("#report").load(urls)
				} else {
					<?php if(isset($js) && $js==0) { ?>
					var urls = '<?php echo $url_data ?>/1/'+ month +'/'+year;
					<?php } else { ?>
					var urls = '<?php echo $url_data ?>/1/'+ id +'/'+year;
					<?php } ?>
					//alert(urls);
