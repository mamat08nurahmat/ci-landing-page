	...
	$(function() {
		...
		function getReport(ex){
			...
			if(msg){alert(msg); return false;}
				...
				//alert(start+' s/d '+end);
				if(ex == 0){
					var urls = '<?php echo site_url('/report/getdplk/')?>/'+id + '/'+<?php echo $type ?>+ '/' +start+'/'+end; 
					$("#report").html("<img src='<?php echo LOAD ?>'> <span style='color:#080'>loading</span>");
					$("#report").load(urls)
				} else {
					//var urls = '<?php echo site_url('/report/xlsdplk/')?>/'+id + '/'+<?php echo $type ?>+'/' +start+'/'+end;  
					//alert(urls);
