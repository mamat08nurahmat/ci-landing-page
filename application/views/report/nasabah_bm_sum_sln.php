...
<script type="text/javascript">
	$(function() {
		...
		$('#frmCustomer').submit(function(){
			...
			//var id = 34359;
			//var id = '24660';
				var urls = '<?php echo site_url('/report/get_nasabah_bm_tab_sum_sln/')?>/'+ '/' +month + '/' +tipe; 
			$.ajax({
				type:'post',
	...
	var today=new Date();
		...
		function get_detailwil(branch)
		{
			...
			var urls = '<?php echo site_url('/report/get_nasabah_bm_tab_sum/')?>/'+branch + '/' +month + '/' +tipe; 
				$("#reportdetailwil").html("<img src='<?php echo LOAD ?>'> <span style='color:#080'>loading</span>");
				$("#reportdetailwil").load(urls);
				$('#reportdetailwil').dialog('open');
		}
		function get_detailsln(region)
		{
			var year = '<?php echo date('Y');?>';
			var bulan = '<?php echo date('n');?>';
			var month = $('#MONTH').val(); 
			var tipe = $('#TIPE').val();
			//var branch = '<?php echo $this->session->userdata('BRANCH_ID'); ?>';
			var urls = '<?php echo site_url('/report/get_nasabah_bm_tab_sum_wil/')?>/'+region +'/' +month + '/' +tipe; 
				$("#reportdetailsln").html("<img src='<?php echo LOAD ?>'> <span style='color:#080'>loading</span>");
				$("#reportdetailsln").load(urls);
				$('#reportdetailsln').dialog('open');
		}
</script>
...
<div id='content_x'>
	<div id="tabs">
		...
		<div id='reportdetailsln'></div>
		
		<div id='reportdetailwil'></div>
